...
@interface WHImageManager ()
@property (nonatomic, strong) ALAssetsLibrary *assetLibrary;
@end

@implementation WHImageManager

+ (instancetype)manager {
    static WHImageManager *manager;
...
/// Get Album 获得相册/相册数组
- (void)getCameraRollAlbum:(BOOL)allowPickingVideo completion:(void (^)(WHAlbumModel *))completion{
    __block WHAlbumModel *model;
    if (iOS8Later) {
...
- (void)getAllAlbums:(BOOL)allowPickingVideo completion:(void (^)(NSArray<WHAlbumModel *> *))completion{
    ...
    if (iOS8Later) {
        ...
        for (PHAssetCollection *collection in smartAlbums) {
            ...
            if ([collection.localizedTitle isEqualToString:@"Camera Roll"] || [collection.localizedTitle isEqualToString:@"相机胶卷"]) {
                ...
                self.savePhotosModel = [self modelWithResult:fetchResult name:collection.localizedTitle];
            } else {
...
/// Get Asset 获得照片数组
- (void)getAssetsFromFetchResult:(id)result allowPickingVideo:(BOOL)allowPickingVideo completion:(void (^)(NSArray<WHAssetModel *> *))completion {
    NSMutableArray *photoArr = [NSMutableArray array];
    if ([result isKindOfClass:[PHFetchResult class]]) {
        for (PHAsset *asset in result) {
            WHAssetModelMediaType type = WHAssetModelMediaTypePhoto;
            if (asset.mediaType == PHAssetMediaTypeVideo)      type = WHAssetModelMediaTypeVideo;
            else if (asset.mediaType == PHAssetMediaTypeAudio) type = WHAssetModelMediaTypeAudio;
            else if (asset.mediaType == PHAssetMediaTypeImage) {
                if (asset.mediaSubtypes == PHAssetMediaSubtypePhotoLive) type = WHAssetModelMediaTypeLivePhoto;
            }
            NSString *timeLength = type == WHAssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",asset.duration] : @"";
            timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
            [photoArr addObject:[WHAssetModel modelWithAsset:asset type:type timeLength:timeLength]];
        }
        ...
        [gruop enumerateAssetsUsingBlock:^(ALAsset *result, NSUInteger index, BOOL *stop) {
            ...
            if (!allowPickingVideo){
                [photoArr addObject:[WHAssetModel modelWithAsset:result type:type]];
                return;
            }
            /// Allow picking video
            if ([[result valueForProperty:ALAssetPropertyType] isEqualToString:ALAssetTypeVideo]) {
                type = WHAssetModelMediaTypeVideo;
                NSTimeInterval duration = [[result valueForProperty:ALAssetPropertyDuration] integerValue];
                NSString *timeLength = [NSString stringWithFormat:@"%0.0f",duration];
                timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
                [photoArr addObject:[WHAssetModel modelWithAsset:result type:type timeLength:timeLength]];
            } else {
                [photoArr addObject:[WHAssetModel modelWithAsset:result type:type]];
            }
        }];
...
- (void)getPhotosBytesWithArray:(NSArray *)photos completion:(void (^)(NSString *totalBytes))completion {
    ...
    for (NSInteger i = 0; i < photos.count; i++) {
        WHAssetModel *model = photos[i];
        if ([model.asset isKindOfClass:[PHAsset class]]) {
            [[PHImageManager defaultManager] requestImageDataForAsset:model.asset options:nil resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
                if (model.type != WHAssetModelMediaTypeVideo) dataLength += imageData.length;
                if (i >= photos.count - 1) {
            ...
            if (model.type != WHAssetModelMediaTypeVideo) dataLength += (NSInteger)representation.size;
            if (i >= photos.count - 1) {
                NSString *bytes = [self getBytesFromDataLength:dataLength];
                if (completion) completion(bytes);
            }
...
- (void)getPhotoWithAsset:(id)asset photoWidth:(CGFloat)photoWidth completion:(void (^)(UIImage *, NSDictionary *))completion {
    if ([asset isKindOfClass:[PHAsset class]]) {
        ...
        [[PHImageManager defaultManager] requestImageForAsset:asset targetSize:CGSizeMake(pixelWidth, pixelHeight) contentMode:PHImageContentModeDefault options:nil resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
...
- (void)getPostImageWithAlbumModel:(WHAlbumModel *)model completion:(void (^)(UIImage *))completion {
    if (iOS8Later) {
        [[WHImageManager manager] getPhotoWithAsset:[model.result lastObject] photoWidth:80 completion:^(UIImage *photo, NSDictionary *info) {
...
- (WHAlbumModel *)modelWithResult:(id)result name:(NSString *)name{
    WHAlbumModel *model = [[WHAlbumModel alloc] init];
    model.result = result;
